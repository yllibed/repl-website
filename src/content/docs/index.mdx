...
<CardGrid stagger>
	...
	<Card title="Build an MCP server in one line" icon="puzzle">
		`app.UseMcpServer()` lets your app expose every command over the Model Context Protocol — the open standard AI assistants use to discover and call tools. Repl.Mcp is the component; your app is the MCP server. Tools, resources, prompts, and MCP Apps (`ui://`) are generated from the same command graph, with behavioral hints derived from your route annotations.
	</Card>
...
Repl is a complete command surface toolkit. It implements the working [CLI guidelines from clig.dev](https://clig.dev) — the de facto reference for modern command-line tools — out of the box, then extends them for the era of AI agents and multi-channel automation.
...
<CardGrid>
	...
	<Card title="For Coding Agents" icon="puzzle">
		Decision rules, implementation guidance, and copy/paste instructions for agents adding Repl Toolkit to .NET projects.

		[Guide agents →](/getting-started/for-coding-agents/)
	</Card>
