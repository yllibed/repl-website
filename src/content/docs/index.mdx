...
<CardGrid stagger>
	...
	<Card title="Hosted sessions" icon="laptop">
		The same command surface runs over WebSocket or Telnet. Multiple concurrent sessions, each with its own scope. Add it to an ASP.NET Core app with `app.MapReplWebSocket("/repl")`.
	</Card>
...
**When the command surface is complete, the application is complete.** The business logic is implemented, tested, and working. Adding a web UI, a TUI, or a desktop front-end is the last step — decoration on top of a finished application. By then, the real work is done.
