...
### Push filtering and sorting to the data source

Return only the rows the user asked for. The `filter:` predicate on `FromOffset` and `FromAsyncEnumerable` is a fallback for stores that cannot filter server-side — it fetches and discards rows that never reach the terminal.

```csharp
// Preferred — filter at the source
ReplPageSource.FromOffset<Contact, (ContactStore store, string? search)>(
    (store, search),
    static (state, offset, take, ct) =>
        state.store.QueryAsync(offset, take, filter: state.search, ct))

// Fallback — client-side, acceptable when the store cannot filter
...
The same principle applies to sorting — pass `ORDER BY` into your query rather than sorting in-memory after retrieval.

### Prefer page sources over single pages for interactive use

`IReplPageSource<T>` lets the integrated terminal pager continue fetching pages in the same command run. With `ReplPage<T>`, the user must rerun the command with `--result:cursor`. Use `ReplPage<T>` only when the caller explicitly drives pagination (e.g., MCP agents, scripts).

### Do not confuse source paging with output paging

Source paging controls how many rows are fetched from the backing store. Output paging controls how many lines the terminal displays at once. Returning a 10,000-row list and relying on the pager still allocates and formats all 10,000 rows before the first screen appears.
...
<ApiRef uid="Repl.IReplIoContext" /> is injectable as a handler parameter. `io.Output` is always the correct writer for the active session, regardless of execution mode:
...
Use <ApiRef uid="Repl.ReplApp" /> (from the `Repl` meta-package) unless you're in an environment where DI is unavailable or you want zero dependencies. <ApiRef uid="Repl.CoreReplApp" /> (from `Repl.Core`) is the dependency-free base — great for embedding in a larger host.
