...
{
    HostedServiceLifecycleMode = HostedServiceLifecycleMode.Head,
});
```
...
<Aside type="tip">
Use `Results.Cancelled(...)` (exit code 3) when a user-initiated cancellation happens — it's distinct from `Results.Error(code, message)` (exit code 1) for program errors and `Results.Validation(message)` (exit code 2) for bad input.
</Aside>
