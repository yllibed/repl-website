...
12. Exit Code                   — 0 success, 1 failure, 2 validation, 3 cancelled, 5 exception
```
...
app.Use(async (context, next) =>
{
    ...
    await next();
    context.Logger.LogInformation("Command {Route} completed in {Elapsed}ms",
        context.Route, sw.ElapsedMilliseconds);
});
```

Middleware runs between argument binding (step 7) and your handler (step 9). Chain multiple middleware calls — they execute in registration order.
...
app.Use(async (context, next) =>
{
    if (context.Route.StartsWith("admin") && !context.Session.IsAuthenticated)
    {
        context.SetResult(Results.Error("Not authenticated. Run 'auth login' first."));
        return;
    }
    await next();
});
...
app.Use(async (context, next) =>
{
    using var scope = context.Logger.BeginScope(new { context.Route, context.SessionId });
    context.Logger.LogDebug("Executing {Route}", context.Route);
    try
    {
        await next();
    }
    ...
    {
        context.Logger.LogError(ex, "Command {Route} threw", context.Route);
        throw;
    }
});
```
...
## Integrating with `IHostedService`

Run Repl commands from background services or scheduled jobs:

```csharp
public class NightlyCleanup : BackgroundService
{
    private readonly IReplExecutor _executor;

    public NightlyCleanup(IReplExecutor executor) => _executor = executor;
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            await _executor.ExecuteAsync("contacts purge-inactive --confirm", ct);
        }
    }
}
```
...
{
    if (amount <= 0)
        return Results.Error("Amount must be positive.", exitCode: 2);

    var result = await payments.ChargeAsync(amount);

    return result.IsSuccess
        ? Results.Success($"Paid {amount:C}.")
        : Results.Error($"Payment failed: {result.Message}");
});
```
...
<Aside type="tip">
Use `Results.Cancelled(...)` (exit code 3) when a user-initiated cancellation happens — it's distinct from `Results.Error(...)` (exit code 2), which indicates a program error.
</Aside>
