...
**Tell the agent about paging in the tool description.** When a command is paged, its description should mention it — and tell the agent how to continue. Agents do not read `--help`; they read the `description` and `details` fields in the MCP tool schema.

```csharp
app.Map("contacts list", handler)
    .WithDescription("List all contacts, paginated.")
    .WithDetails("""
        Returns up to `_replPageSize` contacts per page (default: 25).
        To continue, call this tool again with `_replCursor` set to `pageInfo.nextCursor`
        from the previous response. When `pageInfo.hasMore` is false, all results have been returned.
        """);
```

Without this guidance, some agents page once and stop, assuming the first page is the complete result.
