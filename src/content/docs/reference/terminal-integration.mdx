...
For sessions over a stream pair (WebSocket, named pipe, custom transport), Repl sends `ESC[c` (Device Attributes) followed by `ESC[18t` (DTTERM window size request) immediately after connection. The terminal's response to `ESC[8;rows;cols t` is parsed as the initial size.

Resize events — when the user resizes the terminal window during a session — are delivered as the same `ESC[8;rows;cols t` sequence and processed continuously.
