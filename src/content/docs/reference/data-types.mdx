...
|---|---|---|
| <ApiRef uid="Repl.ReplDateRange" /> | `DateOnly` | Duration must be whole days (`d` only, no sub-day units) |
| <ApiRef uid="Repl.ReplDateTimeRange" /> | `DateTime` | Any `TimeSpan` format |
| <ApiRef uid="Repl.ReplDateTimeOffsetRange" /> | `DateTimeOffset` | Any `TimeSpan` format |

All three types expose `.From` and `.To` properties of their respective type.

<Aside type="note">
  Temporal range types are used as handler parameter types, not as constraint names. Repl infers the binding automatically when the parameter type is <ApiRef uid="Repl.ReplDateRange" />, <ApiRef uid="Repl.ReplDateTimeRange" />, or <ApiRef uid="Repl.ReplDateTimeOffsetRange" />.
</Aside>
