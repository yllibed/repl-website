...
| `.ReadOnly()` | Tool + auto-promoted Resource | Route becomes a `resource://` URI |
| `.AsResource(uri)` | Resource only | Not callable as tool |
...
.Destructive()           // warn before calling
.RequiresConfirmation()  // ask user before calling
.IdempotentOnRetry(false); // not safe to retry
```
...
app.Map("contacts export", (IContactStore store) => store.All())
    .AsResource("contacts://all", "application/json")
...
// Registered as both: tool 'contacts_id_show' AND resource 'contacts://{id}/show'
```
...
app.Map("contacts dashboard", (IContactStore store) =>
{
    ...
    return $$"""
        ...
        <body>
          <h1>Contacts</h1>
          <ul>{{string.Join("", contacts.Select(c => $"<li><b>{c.Name}</b> — {c.Email}</li>"))}}</ul>
        </body>
        </html>
        """;
})
...
Repl routes `IInteraction` calls through MCP primitives when running as an MCP server. Configure how it falls back when the client doesn't support elicitation:

```csharp
app.UseMcpServer(o =>
{
    o.InteractivityMode = InteractivityMode.PrefillThenElicitation;
    //  PrefillThenFail        — fail if no prefill provided
...
**Design for agents:** prefer accepting all required data as route parameters or named options. Reserve `IInteraction` prompts for the human-interactive experience — agents should pass everything upfront.
...
`IMcpClientRoots` exposes the workspace boundaries that the MCP client reports (opened folders, project roots):

```csharp
app.Map("analyze {path}", async (string path, IMcpClientRoots roots, CancellationToken ct) =>
{
    if (!roots.IsAllowed(path))
        return Results.Error($"'{path}' is outside your workspace roots.");

    return await AnalyzeFile(path, ct);
});
```

For clients without native roots support, provide a setup command that stores a soft root in session state:

```csharp
app.Map("init {path}", (string path, IMcpClientRoots roots) =>
{
    roots.SetSoftRoots([new Uri($"file:///{path}")]);
    roots.InvalidateRouting();  // rebuild tool graph with new context
    return Results.Success($"Working directory set to {path}.");
});
```
...
Some MCP clients have limited tool discovery. Enable shim mode for maximum compatibility:
...
The agent first sees only `discover_tools` and `call_tool`. After calling `discover_tools`, it receives the full tool list via `notifications/tools/list_changed`.
...
app.UseMcpServer(o =>
{
    ...
    o.InteractivityMode        = InteractivityMode.PrefillThenElicitation;
    o.ResourceFallbackToTools  = true;   // ReadOnly() tools also registered as resources
    o.PromptFallbackToTools    = true;   // AsPrompt() tools also callable as tools
    o.CommandFilter            = cmd => true;
    o.DynamicToolCompatibility = DynamicToolCompatibilityMode.Default;
});
```
