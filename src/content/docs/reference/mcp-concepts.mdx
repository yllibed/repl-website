...
Repl routes `IReplInteractionChannel` calls through MCP primitives when running as an MCP server. Configure how it falls back when the client doesn't support elicitation:
...
app.Map("analyze {path}", async (string path, IMcpClientRoots roots, CancellationToken ct) =>
{
    if (!roots.IsAllowed(path))
        return Results.Error("outside_root", $"'{path}' is outside your workspace roots.");
...
{
    // Normalize to absolute form; root-containment is enforced by IsAllowed below
    var canonical = Path.GetFullPath(path);
    roots.SetSoftRoots([new Uri(canonical)]);
