...
{
    o.DefaultFormat = "json";  // all commands default to JSON
});
```
...
app.Configure<OutputOptions>(o =>
{
    o.AnsiMode = AnsiMode.TrueColor;   // force 24-bit
...
app.Configure<OutputOptions>(o =>
{
    o.AddTransformer("csv", new CsvOutputTransformer());
    o.AddAlias("spreadsheet", "csv");  // both --output:format=csv and --output:format=spreadsheet work
});
```

```csharp
public class CsvOutputTransformer : IReplOutputTransformer
{
    public async Task TransformAsync(object? value, ReplOutputContext context, CancellationToken ct)
    {
        if (value is IEnumerable<object> rows)
        {
            await context.Writer.WriteLineAsync(ToCsv(rows));
        }
    }
}
```
...
app.Configure<OutputOptions>(o =>
{
    ...
    // or: BannerFormats = ["human"]  — show only in human format (default)
});
```
...
Add `Repl.Spectre` and register the services:
