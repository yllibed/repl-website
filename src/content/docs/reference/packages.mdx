...
All packages target **.NET 8+** and are published to NuGet under the `Repl` family.
...
- `app.MapReplWebSocket("/repl")` on an ASP.NET Core app
- Per-session scope, shared singletons, terminal metadata
- Session registry
...
Telnet session host (Telnet framing over WebSocket):

- `app.MapReplTelnet("/telnet")`
- NAWS window-size negotiation
- Compatible with Telnet clients via WebSocket bridge
