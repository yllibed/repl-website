...
Implicit types not available as constraint names: `FileInfo`, `DirectoryInfo`, <ApiRef uid="Repl.ReplDateRange" />, <ApiRef uid="Repl.ReplDateTimeRange" />, <ApiRef uid="Repl.ReplDateTimeOffsetRange" />.
...
<ApiRef uid="Repl.ReplDateRange" />, <ApiRef uid="Repl.ReplDateTimeRange" />, and <ApiRef uid="Repl.ReplDateTimeOffsetRange" /> capture an interval from a single token. The type is inferred from the handler parameter — no constraint name needed.
...
2. **Explicit attributes** — <ApiRef uid="Repl.FromServicesAttribute" label="[FromServices]" />, <ApiRef uid="Repl.FromContextAttribute" label="[FromContext]" />
3. **Options groups** — classes decorated with <ApiRef uid="Repl.Parameters.ReplOptionsGroupAttribute" label="[ReplOptionsGroup]" />
...
app.Map("list", (SortOptions sort, IStore store) => store.List(sort.SortBy, sort.Descending));
app.Map("search {q}", (string q, SortOptions sort, IStore store) => store.Search(q, sort.SortBy));
```

Both commands now share `--sort-by` and `--descending` without duplicating them.

<Aside type="note">
For paging large result sets, Repl has a first-class mechanism that handles cursors, terminal UX, and MCP automatically. See [Result-Flow Paging](/reference/result-flow-paging/).
</Aside>
...
Override with <ApiRef uid="Repl.Parameters.ReplOptionAttribute" label="[ReplOption(Mode = ReplParameterMode.OptionOnly)]" /> or <ApiRef uid="Repl.Parameters.ReplArgumentAttribute" label="[ReplArgument(Mode = ...)]" />.
