...
app.Map("contact add {name?} {email?:email}", static (string? name, string? email) => ...);
```
...
```text
myapp co li         # → contact list
myapp co 42 sh      # → contact 42 show
```

In the REPL:
```text
> co                # enters [client] context
[client]> li        # runs list
...
Literal segment matching is case-insensitive. `Contact`, `contact`, and `CONTACT` all resolve to the same route.
...
- **Typed dynamic segments** score by constraint specificity: `int` > `long` > `guid` > `bool` > `alpha` > `string`
- Routes that match more segments score higher overall

```csharp
app.Map("contact list",         static (...) => ...);  // literal "list"
app.Map("contact {id:int} show", static (...) => ...);  // int param + literal "show"
app.Map("contact {id:int} remove", static (...) => ...); // int param + literal "remove"
```

| Input | Route matched | Reason |
|-------|---------------|--------|
| `contact list` | `contact list` | `list` is a literal — scores higher than `{id:int}` for that token |
| `contact 42 show` | `contact {id:int} show` | `42` satisfies `:int`; literal `show` confirms the route |
| `co li` | `contact list` | `co` → `contact`, `li` → `list` via prefix expansion |
| `co 42 sh` | `contact {id:int} show` | `co` → `contact`, `42` → `:int` match, `sh` → `show` |
...
```text
> contac lis
error: no command found. Did you mean: contact list?
```
