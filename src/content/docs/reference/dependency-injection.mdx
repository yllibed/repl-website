...
builder.Services.AddSingleton(typeof(IStore<>), typeof(InMemoryStore<>));  // open generic
builder.Services.AddScoped<ICurrentUser, HttpContextUser>();               // per-session

var app = builder.Build().UseDefaultInteractive();
```

Or with the factory overload:
...
| `Scoped` | One instance per session (in hosted sessions) or per invocation (in CLI mode) |
