...
Any other transport — SSH, named pipes, TCP sockets, gRPC streams — works by instantiating <ApiRef uid="Repl.StreamedReplHost" /> with a `TextWriter`, pushing received text via `EnqueueInput`, and calling `RunSessionAsync`.
...
Each session carries terminal metadata — capabilities and window size, injectable via <ApiRef uid="Repl.Interaction.ITerminalInfo" /> (namespace `Repl.Interaction`):
...
- [Dependency Injection](/reference/dependency-injection/) — service lifetimes and how `AddScoped` maps to per-session state in hosted sessions.
- [Terminal Integration](/reference/terminal-integration/) — terminal capability detection, ANSI support, and window-size negotiation.
- [Pipelining & Lifecycle](/reference/pipelining/) — hosted service integration and <ApiRef uid="Repl.ReplRunOptions" />.
