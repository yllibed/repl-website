...
For local samples, prefer absolute project paths in host configs:
...
Use `.cursor/mcp.json` for Cursor, `.vscode/mcp.json` for VS Code/Copilot, Claude Code's MCP registration/settings flow, or Cline's MCP settings. Review commands before enabling them: local MCP servers run code on the user's machine.
