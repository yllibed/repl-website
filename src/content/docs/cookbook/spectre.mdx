...
`UseSpectreConsole()` upgrades all <ApiRef uid="Repl.Interaction.IReplInteractionChannel" /> prompts to Spectre.Console widgets automatically:
...
- [Interactivity](/reference/interactivity/) — the <ApiRef uid="Repl.Interaction.IReplInteractionChannel" /> API and how Spectre upgrades each prompt type.
- [Best Practices](/reference/best-practices/) — output routing, why `AnsiConsole` static must be avoided, and `IAnsiConsole` injection.
