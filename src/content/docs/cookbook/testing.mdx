...
`RunCommandAsync` returns a <ApiRef uid="Repl.Testing.CommandExecution" /> with the output text, exit code, and the raw result object:
...
- [Best Practices](/reference/best-practices/) — test-first approach, testing at the command level, and interaction supply patterns.
- [Pipelining & Lifecycle](/reference/pipelining/) — exit codes and how `Results.*` maps to observable outcomes.
