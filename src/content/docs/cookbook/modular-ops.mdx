...
app.Context("contacts", contacts =>
{
    contacts.Map("list", (IStore<Contact> store) => store.All());
    contacts.Map("show {id:int}", (int id, IStore<Contact> store) => store.Get(id));
});
```

## Conditional modules

Mount a module only when a certain channel capability is present:

```csharp
app.Context("admin", admin =>
{
    admin.Map("purge-all", (IStore<Contact> store) => store.DeleteAll());
})
.WithChannelFilter(c => c.IsLocalConsole); // only available in local interactive sessions
```
