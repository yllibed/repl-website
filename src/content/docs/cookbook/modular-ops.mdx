...
A module is a method (or class) that registers commands onto an <ApiRef uid="Repl.IReplMap" />:
...
Use `MapModule(..., predicate)` when an entire module should be available only for selected runtime channels:

```csharp
sealed class AdminModule : IReplModule
{
    public void Map(IReplMap map)
    {
        map.Context("admin", admin =>
        {
...
app.MapModule(
    new AdminModule(),
    static context => context.Channel != ReplRuntimeChannel.Programmatic);
```

The predicate receives a <ApiRef uid="Repl.ModulePresenceContext" />, so it can inspect the active <ApiRef uid="Repl.ReplRuntimeChannel" /> and session state before the module is included in the command graph.

For a single human-only command, keep the module mounted and call <ApiRef uid="Repl.CommandBuilder.AutomationHidden(System.Boolean)" label=".AutomationHidden()" /> on the command builder returned by `Map(...)`.
...
- [Modules](/reference/modules/) — complete `MapModule` API: DI-resolved generic form, instance form, conditional and DI-injected predicates, dynamic context routes.
- [Dependency Injection](/reference/dependency-injection/) — service lifetimes and how services are resolved inside module constructors and handlers.
- [Best Practices](/reference/best-practices/) — when to extract a module and how to keep the command graph maintainable.
